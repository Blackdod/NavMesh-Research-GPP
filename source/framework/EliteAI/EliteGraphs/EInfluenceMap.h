...
namespace Elite
{
	...
	void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		...
		{
			float currentInfluence = m_Nodes[i]->GetInfluence();
			float highestInfluence = 0;
			for (const auto& connection : GetNodeConnections( m_Nodes[i] ))
			{
				const auto neighbor{ m_Nodes[connection->GetTo()] };
				float newInfluence = neighbor->GetInfluence() * expf(-connection->GetCost() * m_Decay);
				if (abs(newInfluence) > abs(highestInfluence))
				{
					highestInfluence = newInfluence;
				}

				m_InfluenceDoubleBuffer[i] = Lerp(currentInfluence, highestInfluence, m_Momentum);
			}
		}
