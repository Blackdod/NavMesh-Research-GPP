	...
	public:
		...
		static std::vector<Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			for (unsigned int portalIdx = 1; portalIdx < amtPortals; ++portalIdx)
			{
				...
				if(Cross(newRightLeg, rightLeg) <= 0.f)
				{
					//2. See if new line degenerates a line segment - RIGHT
					if(Cross(newRightLeg, leftLeg) > 0.f)
					{
					...
					{
						apexPos += leftLeg; //move apex along left leg
						apexIdx = leftLegIdx;

						//start from next idx
						const unsigned int newIt{ static_cast<unsigned int>(apexIdx) + 1}; 
						portalIdx = newIt;
						leftLegIdx = newIt;
						rightLegIdx = newIt;
				...
				if (Cross(newLeftLeg, leftLeg) >= 0.f)
				{
					...
					if (Cross(newLeftLeg, rightLeg) < 0.f)
					{
						leftLeg = newLeftLeg;
					...
					{
						apexPos += rightLeg; //move apex along right leg
						apexIdx = rightLegIdx;
