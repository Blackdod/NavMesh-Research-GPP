...
void App_Flocking::Start()
{
	...
	m_pAgentToEvade = new SteeringAgent(2.f);
	m_pAgentToEvade->SetPosition(Vector2{ m_TrimWorldSize/2, m_TrimWorldSize/2 });
	m_pAgentToEvade->SetSteeringBehavior(m_pSeekBehavior);
	m_pAgentToEvade->SetBodyColor({ 1,0,0 });
	m_pAgentToEvade->SetAutoOrient(true);
	m_pAgentToEvade->SetMass(2.f);
	m_pAgentToEvade->SetMaxLinearSpeed(25.f);
...
void App_Flocking::Update(float deltaTime)
{
	...
	m_pSeekBehavior->SetTarget(m_MouseTarget);
}
